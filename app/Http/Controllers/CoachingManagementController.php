...
class CoachingManagementController extends Controller
{
    public function index(Request $request, QaAccessService $access): Response
    {
        ...
        return Inertia::render('coaching/index', [
            ...
            'coaches' => User::query()->whereIn('role', ['admin', 'manager', 'qa_admin'])->when($request->user()->role === 'team_leader', fn ($q) => $q->whereIn('id', (clone $base)->select('coach_id')))->orderBy('name')->get(['id', 'name']),
            ...
            'prefill' => in_array($request->user()->role, ['admin', 'manager', 'qa_admin'], true) ? $this->coachingPrefill($request) : [],
            'can_manage' => in_array($request->user()->role, ['admin', 'manager', 'qa_admin'], true),
        ]);
    }
    ...
    public function show(Request $request, CoachingRecord $coaching, QaAccessService $access): Response
    {
        ...
        return Inertia::render('coaching/show', ['coaching' => $this->detail($coaching), 'materials' => $request->user()->role === 'team_leader' ? [] : TrainingLibraryMaterial::query()->where('status', 'active')->orderBy('title')->get(['id', 'title', 'type', 'skill_id']), 'can_manage' => in_array($request->user()->role, ['admin', 'manager', 'qa_admin'], true)]);
    }
